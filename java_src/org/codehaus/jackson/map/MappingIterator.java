...
public class MappingIterator<T> implements Iterator<T> {
    ...
    public MappingIterator(JavaType javaType, JsonParser jsonParser, DeserializationContext deserializationContext, JsonDeserializer<?> jsonDeserializer) {
        ...
        if (jsonParser == null || jsonParser.getCurrentToken() != JsonToken.START_ARRAY || jsonParser.mo40171getParsingContext().inRoot()) {
            return;
        }
    ...
    public T nextValue() {
        T mo40306deserialize = this._deserializer.mo40306deserialize(this._parser, this._context);
