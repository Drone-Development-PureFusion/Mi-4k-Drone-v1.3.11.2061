package cn.sharesdk.facebook;

import android.content.Intent;
import android.content.pm.PackageManager;
import android.content.pm.ResolveInfo;
import android.content.pm.Signature;
import android.text.TextUtils;
import cn.sharesdk.framework.authorize.AbstractC0789d;
import cn.sharesdk.framework.authorize.C0788c;
import com.tencent.connect.common.Constants;
/* renamed from: cn.sharesdk.facebook.b */
/* loaded from: classes.dex */
public class C0769b extends AbstractC0789d {

    /* renamed from: d */
    private String f1447d;

    /* renamed from: e */
    private String[] f1448e;

    public C0769b(C0788c c0788c) {
        super(c0788c);
    }

    /* renamed from: b */
    private boolean m38064b() {
        boolean z = true;
        Intent intent = new Intent();
        intent.setClassName("com.facebook.katana", "com.facebook.katana.ProxyAuth");
        intent.putExtra(Constants.PARAM_CLIENT_ID, this.f1447d);
        if (this.f1448e != null && this.f1448e.length > 0) {
            intent.putExtra("scope", TextUtils.join(com.xiaomi.mipush.sdk.Constants.ACCEPT_TIME_SEPARATOR_SP, this.f1448e));
        }
        if (!m38063b(intent)) {
            return false;
        }
        try {
            this.f1516a.startActivityForResult(intent, this.f1517b);
        } catch (Throwable th) {
            z = false;
        }
        return z;
    }

    /* renamed from: b */
    private boolean m38063b(Intent intent) {
        ResolveInfo resolveActivity = this.f1516a.getContext().getPackageManager().resolveActivity(intent, 0);
        if (resolveActivity == null) {
            return false;
        }
        try {
            Signature[] signatureArr = this.f1516a.getContext().getPackageManager().getPackageInfo(resolveActivity.activityInfo.packageName, 64).signatures;
            for (Signature signature : signatureArr) {
                if ("".equals(signature.toCharsString())) {
                    return true;
                }
            }
            return false;
        } catch (PackageManager.NameNotFoundException e) {
            return false;
        }
    }

    /* renamed from: c */
    private void m38062c(Intent intent) {
        if (this.f1518c == null) {
            return;
        }
        String stringExtra = intent.getStringExtra("error_message");
        if (stringExtra == null) {
            stringExtra = intent.getStringExtra("error_code");
        }
        if (stringExtra == null) {
            this.f1518c.onComplete(intent.getExtras());
        } else if (stringExtra.equals("access_denied") || stringExtra.equals("OAuthAccessDeniedException")) {
            this.f1518c.onCancel();
        } else {
            String stringExtra2 = intent.getStringExtra("error_message");
            if (stringExtra2 != null) {
                stringExtra = intent.getStringExtra("error_code") + ": " + stringExtra2;
            }
            this.f1518c.onFailed(new Throwable(stringExtra));
        }
    }

    /* renamed from: d */
    private void m38061d(Intent intent) {
        if (intent == null) {
            if (this.f1518c == null) {
                return;
            }
            this.f1518c.onCancel();
            return;
        }
        String stringExtra = intent.getStringExtra("error");
        Throwable th = new Throwable(stringExtra + " (" + intent.getStringExtra("error_code") + ")");
        if (this.f1518c == null) {
            return;
        }
        this.f1518c.onFailed(th);
    }

    @Override // cn.sharesdk.framework.authorize.AbstractC0789d
    /* renamed from: a */
    public void mo37980a() {
        if (!m38064b()) {
            this.f1516a.finish();
            if (this.f1518c == null) {
                return;
            }
            this.f1518c.onFailed(new Throwable());
        }
    }

    @Override // cn.sharesdk.framework.authorize.AbstractC0789d
    /* renamed from: a */
    public void mo37978a(int i, int i2, Intent intent) {
        this.f1516a.finish();
        if (i == this.f1517b) {
            switch (i2) {
                case -1:
                    m38062c(intent);
                    return;
                case 0:
                    m38061d(intent);
                    return;
                default:
                    return;
            }
        }
    }

    /* renamed from: a */
    public void m38065a(String str, String[] strArr) {
        this.f1447d = str;
        this.f1448e = strArr;
    }
}
