...
public class BitmapProcessor {
    ...
    public static class ManagerThread implements Handler.Callback {
        ...
        public boolean handleMessage(Message message) {
            ...
            MobLog.getInstance().m6200d(">>>> BitmapProcessor.cachePool: " + (BitmapProcessor.cachePool == null ? 0 : BitmapProcessor.cachePool.size()), new Object[0]);
            MobLog.getInstance().m6200d(">>>> BitmapProcessor.reqList: " + (BitmapProcessor.reqList == null ? 0 : BitmapProcessor.reqList.size()), new Object[0]);
    ...
    private static class PatchInputStream extends FilterInputStream {
        ...
        public long skip(long j) {
            long j2 = 0;
            while (j2 < j) {
                long skip = this.f31135in.skip(j - j2);
                if (skip == 0) {
                    break;
                }
                j2 += skip;
