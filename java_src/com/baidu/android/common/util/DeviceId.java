...
public final class DeviceId {
    ...
    private CUIDInfo getCUIDInfo() {
        ...
        if (collectBuddyInfos == null || collectBuddyInfos.size() == 0) {
            for (int i = 0; i < 3; i++) {
                Log.m36878w(TAG, "galaxy lib host missing meta-data,make sure you know the right way to integrate galaxy");
            }
            z = false;
        } else {
            ...
            if (!cUIDBuddyInfo.sigMatched) {
                for (int i2 = 0; i2 < 3; i2++) {
                    Log.m36878w(TAG, "galaxy config err, In the release version of the signature should be matched");
                }
            }
            z = z3;
        }
        ...
        if (createFromJson == null) {
            ...
            if (collectBuddyInfos2 != null) {
                ...
                if (!C1576a.f3873c.equals(filesDir.getName())) {
                    Log.m36885e(TAG, "fetal error:: app files dir name is unexpectedly :: " + filesDir.getAbsolutePath());
    ...
    */
    private String getIMEIFromSystem(String str) {
        ...
        try {
            ...
            Log.m36884e(TAG, "Read IMEI failed", e);
        }
