...
public class AsyncHttpClient {
            ...
            public void process(HttpRequest httpRequest, HttpContext httpContext) {
                ...
                for (String str : AsyncHttpClient.this.clientHeaderMap.keySet()) {
                    if (httpRequest.containsHeader(str)) {
                        ...
                        AsyncHttpClient.log.mo36211d(AsyncHttpClient.LOG_TAG, String.format("Headers were overwritten! (%s | %s) overwrites (%s | %s)", str, AsyncHttpClient.this.clientHeaderMap.get(str), firstHeader.getName(), firstHeader.getValue()));
    ...
    public static void endEntityViaReflection(HttpEntity httpEntity) {
        ...
        if (httpEntity instanceof HttpEntityWrapper) {
            try {
                ...
                log.mo36208e(LOG_TAG, "wrappedEntity consume", th);
            }
        }
    }
    ...
    private static SchemeRegistry getDefaultSchemeRegistry(boolean z, int i, int i2) {
        if (z) {
            log.mo36211d(LOG_TAG, "Beware! Using the fix is insecure, as it doesn't verify SSL certificates.");
        }
        if (i < 1) {
            i = 80;
            log.mo36211d(LOG_TAG, "Invalid HTTP port number specified, defaulting to 80");
        }
        if (i2 < 1) {
            i2 = 443;
            log.mo36211d(LOG_TAG, "Invalid HTTPS port number specified, defaulting to 443");
        }
    ...
    */
    public static String getUrlWithQueryString(boolean z, String str, RequestParams requestParams) {
        ...
        if (z) {
            try {
                ...
                log.mo36208e(LOG_TAG, "getUrlWithQueryString encoding URL", e);
            }
            if (requestParams != null) {
                return aSCIIString;
            }
            String trim = requestParams.getParamString().trim();
            if (trim.equals("") || trim.equals(C0363a.f360a)) {
                return aSCIIString;
            }
            return (aSCIIString + (aSCIIString.contains(C0363a.f360a) ? "&" : C0363a.f360a)) + trim;
        }
        aSCIIString = str;
        if (requestParams != null) {
        }
    ...
    public static void silentCloseInputStream(InputStream inputStream) {
        if (inputStream != null) {
            try {
                ...
                log.mo36202w(LOG_TAG, "Cannot close input stream", e);
            }
        }
    }
    ...
    public static void silentCloseOutputStream(OutputStream outputStream) {
        if (outputStream != null) {
            try {
                ...
                log.mo36202w(LOG_TAG, "Cannot close output stream", e);
            }
        }
    }
    ...
    public void cancelRequests(Context context, final boolean z) {
        if (context == null) {
            log.mo36209e(LOG_TAG, "Passed null Context to cancelRequests");
            return;
        }
    ...
    public void cancelRequestsByTAG(Object obj, boolean z) {
        if (obj == null) {
            log.mo36211d(LOG_TAG, "cancelRequestsByTAG, passed TAG is null, cannot proceed");
            return;
        }
        for (List<RequestHandle> list : this.requestMap.values()) {
            if (list != null) {
    ...
    protected RequestHandle sendRequest(DefaultHttpClient defaultHttpClient, HttpContext httpContext, HttpUriRequest httpUriRequest, String str, ResponseHandlerInterface responseHandlerInterface, Context context) {
        ...
        if (str != null) {
            if (!(httpUriRequest instanceof HttpEntityEnclosingRequestBase) || ((HttpEntityEnclosingRequestBase) httpUriRequest).getEntity() == null || !httpUriRequest.containsHeader("Content-Type")) {
                ...
                log.mo36203w(LOG_TAG, "Passed contentType will be ignored because HttpEntity sets content type");
            }
        }
    ...
    public void setCookieStore(CookieStore cookieStore) {
        this.httpContext.setAttribute(C11060a.f35613f, cookieStore);
    }

    public void setCredentials(AuthScope authScope, Credentials credentials) {
        if (credentials == null) {
            log.mo36211d(LOG_TAG, "Provided credentials are null, not setting");
            return;
        }
