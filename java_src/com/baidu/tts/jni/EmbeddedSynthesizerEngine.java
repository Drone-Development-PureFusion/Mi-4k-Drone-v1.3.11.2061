...
public class EmbeddedSynthesizerEngine {
    ...
    static {
        try {
            LoggerProxy.m36554d(TAG, "before load gnustl_shared");
            System.loadLibrary("gnustl_shared");
        } catch (Throwable th) {
            LoggerProxy.m36553e(TAG, "so file gnustl_shared load fail");
        }
        try {
            LoggerProxy.m36554d(TAG, "before load BDSpeechDecoder_V1");
            System.loadLibrary("BDSpeechDecoder_V1");
        } catch (Throwable th2) {
            LoggerProxy.m36553e(TAG, "so file BDSpeechDecoder_V1 load fail");
        }
        try {
            LoggerProxy.m36554d(TAG, "before load bd_etts");
            System.loadLibrary("bd_etts");
        } catch (Throwable th3) {
            LoggerProxy.m36553e(TAG, "so file bd_etts load fail");
        }
        try {
            LoggerProxy.m36554d(TAG, "before load bdtts");
            System.loadLibrary("bdtts");
            LoggerProxy.m36554d(TAG, "after load bdtts");
        } catch (Throwable th4) {
            LoggerProxy.m36553e(TAG, "so file bdtts load fail");
        }
    }
