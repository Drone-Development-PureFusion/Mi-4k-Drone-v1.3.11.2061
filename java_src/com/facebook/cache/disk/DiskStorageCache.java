...
public class DiskStorageCache implements FileCache, DiskTrimmable {
    ...
    private void calcFileCacheSize() {
        ...
        try {
            long j4 = 0;
            int i5 = 0;
            for (DiskStorage.Entry entry : this.mStorageSupplier.get().mo40050getEntries()) {
                int i6 = i5 + 1;
                j4 += entry.getSize();
    ...
    private BinaryResource commitResource(String str, CacheKey cacheKey, BinaryResource binaryResource) {
        BinaryResource mo40048commit;
        synchronized (this.mLock) {
            mo40048commit = this.mStorageSupplier.get().mo40048commit(str, binaryResource, cacheKey);
            this.mCacheStats.increment(mo40048commit.size(), 1L);
        }
        return mo40048commit;
    }

    private BinaryResource createTemporaryResource(String str, CacheKey cacheKey) {
        maybeEvictFilesInCacheDir();
        return this.mStorageSupplier.get().mo40049createTemporary(str, cacheKey);
    }
    ...
    private void deleteTemporaryResource(BinaryResource binaryResource) {
        ...
        FLog.m35563e(TAG, "Temp file still on disk: %s ", file);
        if (file.delete()) {
            return;
        }
        FLog.m35563e(TAG, "Failed to delete temp file: %s", file);
    }
    ...
    private void evictAboveSize(long j, CacheEventListener.EvictionReason evictionReason) {
        ...
        try {
            Collection<DiskStorage.Entry> sortedEntries = getSortedEntries(diskStorage.mo40050getEntries());
    ...
    public long clearOldEntries(long j) {
        ...
        synchronized (this.mLock) {
            try {
                ...
                int i2 = 0;
                long j4 = 0;
                for (DiskStorage.Entry entry : diskStorage.mo40050getEntries()) {
    ...
    public BinaryResource getResource(CacheKey cacheKey) {
        BinaryResource mo40051getResource;
        try {
            synchronized (this.mLock) {
                mo40051getResource = this.mStorageSupplier.get().mo40051getResource(getResourceId(cacheKey), cacheKey);
                if (mo40051getResource == null) {
                    this.mCacheEventListener.onMiss();
                } else {
                    this.mCacheEventListener.onHit();
                }
            }
            return mo40051getResource;
        } catch (IOException e) {
    ...
    public BinaryResource insert(CacheKey cacheKey, WriterCallback writerCallback) {
        ...
        try {
            ...
            FLog.m35576d(TAG, "Failed inserting a file into the cache", (Throwable) e);
            throw e;
        }
    }
