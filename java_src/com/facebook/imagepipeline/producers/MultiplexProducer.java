...
public abstract class MultiplexProducer<K, T extends Closeable> implements Producer<T> {
    ...
    /* renamed from: getKey */
    protected abstract K mo40097getKey(ProducerContext producerContext);

    @Override // com.facebook.imagepipeline.producers.Producer
    public void produceResults(Consumer<T> consumer, ProducerContext producerContext) {
        boolean z;
        MultiplexProducer<K, T>.Multiplexer existingMultiplexer;
        K mo40097getKey = mo40097getKey(producerContext);
        do {
            z = false;
            synchronized (this) {
                existingMultiplexer = getExistingMultiplexer(mo40097getKey);
                if (existingMultiplexer == null) {
                    existingMultiplexer = createAndPutNewMultiplexer(mo40097getKey);
                    z = true;
                }
            }
