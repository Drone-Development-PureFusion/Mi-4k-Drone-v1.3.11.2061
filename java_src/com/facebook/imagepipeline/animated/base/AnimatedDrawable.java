...
public class AnimatedDrawable extends Drawable implements DrawableWithCaches, AnimatableDrawable {
    ...
    private final Runnable mNextFrameTask = new Runnable() { // from class: com.facebook.imagepipeline.animated.base.AnimatedDrawable.2
        @Override // java.lang.Runnable
        public void run() {
            FLog.m35540v(AnimatedDrawable.TAG, "(%s) Next Frame Task", AnimatedDrawable.this.mLogId);
    ...
    private final Runnable mInvalidateTask = new Runnable() { // from class: com.facebook.imagepipeline.animated.base.AnimatedDrawable.3
        @Override // java.lang.Runnable
        public void run() {
            FLog.m35540v(AnimatedDrawable.TAG, "(%s) Invalidate Task", AnimatedDrawable.this.mLogId);
    ...
    private final Runnable mWatchdogTask = new Runnable() { // from class: com.facebook.imagepipeline.animated.base.AnimatedDrawable.4
        @Override // java.lang.Runnable
        public void run() {
            FLog.m35540v(AnimatedDrawable.TAG, "(%s) Watchdog Task", AnimatedDrawable.this.mLogId);
    ...
    private void computeAndScheduleNextFrame(boolean z) {
        ...
        FLog.m35538v(TAG, "(%s) Next frame (%d) in %d ms", this.mLogId, Integer.valueOf(i3), Integer.valueOf(timestampMsForFrame));
    ...
    private boolean renderFrame(Canvas canvas, int i, int i2) {
        ...
        if (bitmapForFrame != null) {
            ...
            if (this.mIsRunning && i2 > this.mLastDrawnFrameMonotonicNumber) {
                ...
                if (i3 > 0) {
                    FLog.m35539v(TAG, "(%s) Dropped %d frames", this.mLogId, Integer.valueOf(i3));
                }
            }
            this.mLastDrawnFrame = bitmapForFrame;
            this.mLastDrawnFrameNumber = i;
            this.mLastDrawnFrameMonotonicNumber = i2;
            FLog.m35539v(TAG, "(%s) Drew frame %d", this.mLogId, Integer.valueOf(i));
            return true;
        }
        return false;
    }
    ...
    public C1015q.AbstractC1022b createAnimatorUpdateListener() {
        return new C1015q.AbstractC1022b() { // from class: com.facebook.imagepipeline.animated.base.AnimatedDrawable.5
            @Override // com.p081b.p082a.C1015q.AbstractC1022b
            public void onAnimationUpdate(C1015q c1015q) {
                AnimatedDrawable.this.setLevel(((Integer) c1015q.m37057u()).intValue());
            }
        };
    }

    @Override // com.facebook.imagepipeline.animated.base.AnimatableDrawable
    public C1015q createValueAnimator() {
        int loopCount = this.mAnimatedDrawableBackend.getLoopCount();
        C1015q c1015q = new C1015q();
        c1015q.mo37088a(0, this.mDurationMs);
        c1015q.mo37083b(this.mDurationMs);
        if (loopCount == 0) {
            loopCount = -1;
        }
        c1015q.m37099a(loopCount);
        c1015q.m37084b(1);
        c1015q.mo37097a((Interpolator) new LinearInterpolator());
        c1015q.m37094a(createAnimatorUpdateListener());
        return c1015q;
    }

    @Override // com.facebook.imagepipeline.animated.base.AnimatableDrawable
    public C1015q createValueAnimator(int i) {
        C1015q createValueAnimator = createValueAnimator();
        createValueAnimator.m37099a(Math.max(i / this.mAnimatedDrawableBackend.getDurationMs(), 1));
        ...
        try {
            ...
            if (this.mApplyTransformation) {
                ...
                if (!this.mDstRect.isEmpty()) {
                    AnimatedDrawableCachingBackend mo40071forNewBounds = this.mAnimatedDrawableBackend.mo40071forNewBounds(this.mDstRect);
                    if (mo40071forNewBounds != this.mAnimatedDrawableBackend) {
                        this.mAnimatedDrawableBackend.dropCaches();
                        this.mAnimatedDrawableBackend = mo40071forNewBounds;
                        this.mAnimatedDrawableDiagnostics.setBackend(mo40071forNewBounds);
                    }
            ...
            if (this.mPendingRenderedFrameNumber != -1) {
                ...
                z = false | renderFrame;
                if (renderFrame) {
                    FLog.m35539v(TAG, "(%s) Rendered pending frame %d", this.mLogId, Integer.valueOf(this.mPendingRenderedFrameNumber));
                    this.mPendingRenderedFrameNumber = -1;
                    this.mPendingRenderedFrameMonotonicNumber = -1;
                } else {
                    FLog.m35539v(TAG, "(%s) Trying again later for pending %d", this.mLogId, Integer.valueOf(this.mPendingRenderedFrameNumber));
            ...
            if (this.mPendingRenderedFrameNumber == -1) {
                ...
                z |= renderFrame2;
                if (renderFrame2) {
                    FLog.m35539v(TAG, "(%s) Rendered current frame %d", this.mLogId, Integer.valueOf(this.mScheduledFrameNumber));
                    if (this.mIsRunning) {
                        computeAndScheduleNextFrame(true);
                    }
                } else {
                    FLog.m35539v(TAG, "(%s) Trying again later for current %d", this.mLogId, Integer.valueOf(this.mScheduledFrameNumber));
            ...
            if (!z && this.mLastDrawnFrame != null) {
                ...
                FLog.m35539v(TAG, "(%s) Rendered last known frame %d", this.mLogId, Integer.valueOf(this.mLastDrawnFrameNumber));
                z = true;
            }
            if (!z && (previewBitmap = this.mAnimatedDrawableBackend.getPreviewBitmap()) != null) {
                ...
                previewBitmap.close();
                FLog.m35540v(TAG, "(%s) Rendered preview frame", this.mLogId);
                z = true;
            }
            if (!z) {
                canvas.drawRect(0.0f, 0.0f, this.mDstRect.width(), this.mDstRect.height(), this.mTransparentPaint);
                FLog.m35540v(TAG, "(%s) Failed to draw a frame", this.mLogId);
            }
            canvas.restore();
    ...
    public void dropCaches() {
        FLog.m35540v(TAG, "(%s) Dropping caches", this.mLogId);
        if (this.mLastDrawnFrame != null) {
            this.mLastDrawnFrame.close();
