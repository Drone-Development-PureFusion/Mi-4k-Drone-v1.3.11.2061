...
public class AnimatedDrawableCachingBackendImpl extends DelegatingAnimatedDrawableBackend implements AnimatedDrawableCachingBackend {
    ...
    private Bitmap createNewBitmap() {
        FLog.m35541v(TAG, "Creating new bitmap");
        sTotalBitmaps.incrementAndGet();
        FLog.m35540v(TAG, "Total bitmaps: %d", Integer.valueOf(sTotalBitmaps.get()));
    ...
    private synchronized void doPrefetch(int i, int i2) {
        for (int i3 = 0; i3 < i2; i3++) {
            ...
            if (!hasCachedOrPredecodedFrame && c0018l == null) {
                final C0018l<Object> m39925a = C0018l.m39925a(new Callable<Object>() { // from class: com.facebook.imagepipeline.animated.impl.AnimatedDrawableCachingBackendImpl.3
                ...
                this.mDecodesInFlight.put(frameCount, m39925a);
                m39925a.m39938a((AbstractC0016j<Object, TContinuationResult>) new AbstractC0016j<Object, Object>() { // from class: com.facebook.imagepipeline.animated.impl.AnimatedDrawableCachingBackendImpl.4
                    @Override // p000a.AbstractC0016j
                    /* renamed from: then */
                    public Object mo40096then(C0018l<Object> c0018l2) {
                        AnimatedDrawableCachingBackendImpl.this.onFutureFinished(m39925a, frameCount);
                        return null;
                    }
                });
            }
        ...
        try {
            synchronized (this) {
                ...
                if (cachedOrPredecodedFrame != null) {
                    ...
                    if (now2 > 10) {
                        FLog.m35538v(TAG, "obtainBitmap for frame %d took %d ms (%s)", Integer.valueOf(i), Long.valueOf(now2), "ok");
                    }
                    return cachedOrPredecodedFrame;
                } else if (!z) {
                    long now3 = this.mMonotonicClock.now() - now;
                    if (now3 > 10) {
                        FLog.m35538v(TAG, "obtainBitmap for frame %d took %d ms (%s)", Integer.valueOf(i), Long.valueOf(now3), "deferred");
                    }
                    return null;
                } else {
                    try {
                        ...
                        maybeCacheRenderedBitmap(i, obtainBitmapInternal);
                        CloseableReference<Bitmap> m40058clone = obtainBitmapInternal.m40058clone();
                        obtainBitmapInternal.close();
                        long now4 = this.mMonotonicClock.now() - now;
                        if (now4 > 10) {
                            FLog.m35538v(TAG, "obtainBitmap for frame %d took %d ms (%s)", Integer.valueOf(i), Long.valueOf(now4), "renderedOnCallingThread");
                        }
                        return m40058clone;
                    } catch (Throwable th2) {
                        th = th2;
                        ...
                        if (now5 > 10) {
                            FLog.m35538v(TAG, "obtainBitmap for frame %d took %d ms (%s)", Integer.valueOf(i), Long.valueOf(now5), z2 ? "renderedOnCallingThread" : "ok");
                        }
                        throw th;
                    }
                }
    ...
    private synchronized void maybeCacheRenderedBitmap(int i, CloseableReference<Bitmap> closeableReference) {
        if (this.mBitmapsToKeepCached.get(i)) {
            ...
            this.mCachedBitmaps.put(i, closeableReference.m40058clone());
        }
    }
    ...
    private CloseableReference<Bitmap> obtainBitmapInternal() {
        ...
        return CloseableReference.m35506of(createNewBitmap, this.mResourceReleaserForBitmaps);
    }
    ...
    public synchronized void onFutureFinished(C0018l<?> c0018l, int i) {
        ...
        if (indexOfKey >= 0 && this.mDecodesInFlight.valueAt(indexOfKey) == c0018l) {
            ...
            if (c0018l.m39907f() != null) {
                FLog.m35534v(TAG, c0018l.m39907f(), "Failed to render frame %d", Integer.valueOf(i));
            }
        }
    }
        ...
        synchronized (this) {
            ...
            try {
                if (preDecodedFrame != null) {
                    ...
                    this.mAnimatedImageCompositor.renderFrame(i, obtainBitmapInternal.get());
                    maybeCacheRenderedBitmap(i, obtainBitmapInternal);
                    FLog.m35540v(TAG, "Prefetch rendered frame %d", Integer.valueOf(i));
                    obtainBitmapInternal.close();
                }
            } finally {
    ...
    protected synchronized void finalize() {
        ...
        if (this.mCachedBitmaps.size() > 0) {
            FLog.m35581d(TAG, "Finalizing with rendered bitmaps");
        }
    ...
    public AnimatedDrawableCachingBackend mo40071forNewBounds(Rect rect) {
        AnimatedDrawableBackend mo40071forNewBounds = this.mAnimatedDrawableBackend.mo40071forNewBounds(rect);
        return mo40071forNewBounds == this.mAnimatedDrawableBackend ? this : new AnimatedDrawableCachingBackendImpl(this.mExecutorService, this.mActivityManager, this.mAnimatedDrawableUtil, this.mMonotonicClock, mo40071forNewBounds, this.mAnimatedDrawableOptions);
    }
