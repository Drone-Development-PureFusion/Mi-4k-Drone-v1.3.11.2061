...
public class CountingMemoryCache<K, V> implements MemoryTrimmable, MemoryCache<K, V> {
    ...
    public static class Entry<K, V> {
        ...
        @VisibleForTesting
        /* renamed from: of */
        static <K, V> Entry<K, V> m35503of(K k, CloseableReference<V> closeableReference, @Nullable EntryStateObserver<K> entryStateObserver) {
    ...
    public CountingMemoryCache(ValueDescriptor<V> valueDescriptor, CacheTrimStrategy cacheTrimStrategy, Supplier<MemoryCacheParams> supplier) {
        ...
        this.mMemoryCacheParams = this.mMemoryCacheParamsSupplier.mo40100get();
    }
    ...
    private synchronized CloseableReference<V> newClientReference(final Entry<K, V> entry) {
        ...
        return CloseableReference.m35506of(entry.valueRef.get(), new ResourceReleaser<V>() { // from class: com.facebook.imagepipeline.cache.CountingMemoryCache.2
    ...
    public CloseableReference<V> cache(K k, CloseableReference<V> closeableReference, EntryStateObserver<K> entryStateObserver) {
        ...
        synchronized (this) {
            ...
            if (canCacheNewValue(closeableReference.get())) {
                Entry<K, V> m35503of = Entry.m35503of(k, closeableReference, entryStateObserver);
                this.mCachedEntries.put(k, m35503of);
                closeableReference3 = newClientReference(m35503of);
            } else {
