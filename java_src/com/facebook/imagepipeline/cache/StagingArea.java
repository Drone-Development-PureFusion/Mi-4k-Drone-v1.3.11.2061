...
public class StagingArea {
    ...
    private synchronized void logStats() {
        FLog.m35540v(TAG, "Count = %d", Integer.valueOf(this.mMap.size()));
    }
        ...
        if (encodedImage != null) {
            synchronized (encodedImage) {
                if (!EncodedImage.isValid(encodedImage)) {
                    ...
                    FLog.m35523w(TAG, "Found closed reference %d for key %s (%d)", Integer.valueOf(System.identityHashCode(encodedImage)), cacheKey.toString(), Integer.valueOf(System.identityHashCode(cacheKey)));
                    encodedImage = null;
                } else {
