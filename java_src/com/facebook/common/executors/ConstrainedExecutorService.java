...
public class ConstrainedExecutorService extends AbstractExecutorService {
        ...
        public void run() {
            try {
                ...
                if (runnable != null) {
                    ...
                    FLog.m35540v(ConstrainedExecutorService.TAG, "%s: Worker has nothing to run", ConstrainedExecutorService.this.mName);
                }
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m35539v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(decrementAndGet));
                }
            } catch (Throwable th) {
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m35539v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(decrementAndGet2));
                }
                throw th;
            }
        }
    ...
    public void startWorkerIfNeeded() {
        ...
        while (i < this.mMaxConcurrency) {
            ...
            if (this.mPendingWorkers.compareAndSet(i, i2)) {
                FLog.m35538v(TAG, "%s: starting worker %d of %d", this.mName, Integer.valueOf(i2), Integer.valueOf(this.mMaxConcurrency));
                this.mExecutor.execute(this.mTaskRunner);
                return;
            }
            FLog.m35540v(TAG, "%s: race in startWorkerIfNeeded; retrying", this.mName);
    ...
    public void execute(Runnable runnable) {
        ...
        if (size > i && this.mMaxQueueSize.compareAndSet(i, size)) {
            FLog.m35539v(TAG, "%s: max pending work in queue = %d", this.mName, Integer.valueOf(size));
        }
        startWorkerIfNeeded();
    }
