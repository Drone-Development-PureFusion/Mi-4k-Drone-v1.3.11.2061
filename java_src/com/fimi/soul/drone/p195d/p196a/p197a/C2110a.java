package com.fimi.soul.drone.p195d.p196a.p197a;

import com.fimi.soul.drone.p195d.p196a.AbstractC2195b;
import com.fimi.soul.drone.p195d.p196a.C2196c;
import com.fimi.soul.drone.p195d.p196a.C2197d;
/* renamed from: com.fimi.soul.drone.d.a.a.a */
/* loaded from: classes.dex */
public class C2110a extends AbstractC2195b {

    /* renamed from: a */
    public static final int f6256a = 152;

    /* renamed from: c */
    public static final int f6257c = 3;
    private static final long serialVersionUID = 152;

    /* renamed from: d */
    public short f6258d;

    /* renamed from: e */
    public byte f6259e;

    public C2110a() {
        this.f6910b = 152;
    }

    public C2110a(C2196c c2196c) {
        this.f6910b = 152;
        mo32763a(c2196c.f6914d);
    }

    @Override // com.fimi.soul.drone.p195d.p196a.AbstractC2195b
    /* renamed from: a */
    public void mo32763a(C2197d c2197d) {
        c2197d.m32741c();
        this.f6258d = c2197d.m32738e();
        this.f6259e = c2197d.m32739d();
    }

    @Override // com.fimi.soul.drone.p195d.p196a.AbstractC2195b
    /* renamed from: b */
    public C2196c mo32762b() {
        C2196c c2196c = new C2196c();
        c2196c.f6912b = 3;
        c2196c.f6913c = 152;
        c2196c.f6914d.m32746a(this.f6258d);
        c2196c.f6914d.m32744b(this.f6259e);
        return c2196c;
    }

    public String toString() {
        return "BeginFollowme [Packet_sequence=" + ((int) this.f6258d) + ", Para1=" + ((int) this.f6259e) + "]";
    }
}
