        ...
        if (TextUtils.isEmpty(string)) {
            ...
            if (TextUtils.isEmpty(string2)) {
                ...
                if (stringArrayList != null && stringArrayList.size() > 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (!trim.startsWith("/")) {
                            ...
                            C10257f.m5667e(C10257f.f31917d, "shareToTroopBar(): file_data应该为本地图片");
                            return;
                        } else if (trim.startsWith("/") && !new File(trim).exists()) {
                            iUiListener.onError(new UiError(-6, Constants.MSG_PARAM_ERROR, "图片文件不存在"));
                            C10257f.m5667e(C10257f.f31917d, "shareToTroopBar(): 图片文件不存在");
                            return;
                        }
                    }
                    for (int i2 = 0; i2 < size; i2++) {
                ...
                if (!TextUtils.isEmpty(string3) && !Util.isNumeric(string3)) {
                    ...
                    C10257f.m5667e(C10257f.f31917d, "shareToTroopBar(): troopbar_id 必须为数字");
                    return;
                }
